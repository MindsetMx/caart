...
export class MechanicsComponent implements AfterViewInit {
  ...
  constructor() {
    this.mechanicsForm = this.#fb.group({
      originalRims: ['true', [Validators.required]],
      // rimsDetail: ['', [Validators.required]],
      tireBrand: ['tireBrand', [Validators.required]],
      tireSize: ['tireSize', [Validators.required]],
      tireDate: ['2022-12-31', [Validators.required]],
      tireCondition: ['Excelente', [Validators.required]],
      extraTiresOrRims: ['true', [Validators.required]],
      ...
      originalTransmissionEngine: ['true', [Validators.required]],
      improvementModificationOriginal: ['true', [Validators.required]],
      // whatImprovement: ['', [Validators.required]],
      performedServicesWithDates: ['performedServicesWithDates'],
      mechanicalProblemDetail: ['true', [Validators.required]],
      // whatMechanicalProblem: ['', [Validators.required]],
      illuminatedDashboardSensor: ['true', [Validators.required]],
      // whichIlluminatedSensor: ['', [Validators.required]],
      factoryEquipment: ['true', [Validators.required]],
      extraEquipment: ['true', [Validators.required]],
    ...
    this.#completeCarRegistrationService.getMechanics$(this.originalAuctionCarId).subscribe({
      next: (mechanics) => {
        ...
        const filteredResponse = Object.fromEntries(
          Object.entries(mechanics).filter(([key, value]) => value !== null && value !== undefined && value !== '' && (!Array.isArray(value) || value.length > 0))
        );

        this.mechanicsForm.patchValue(filteredResponse);
